...
namespace mtbit
{
	...
	class BitFlag
	{
		static_assert(  // 列挙型 かつ size_t型の暗黙的型変換ができない！ = enum struct
			std::is_enum<EnumStructT>::value && !std::is_convertible<EnumStructT, size_t>::value,
			"enum structのみ指定できます。");
...
{
	return mtbit::BitFlag<EnumStructT>::New().BeginEdit().On(_e1).On(_e2).EndEdit();
}

template<typename EnumStructT>
mtbit::BitFlag<EnumStructT> operator|(const EnumStructT _e, const mtbit::BitFlag<EnumStructT> _flag)
{
	return _flag.BeginEdit().On(_e).EndEdit();
}
