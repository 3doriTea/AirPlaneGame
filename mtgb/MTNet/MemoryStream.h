...
namespace mtnet
{
	...
	public:
		MemoryStream(mtnet::Byte* _pBuffer, const size_t& _bufferSize);
	...
	private:
		...
		mtnet::Byte* pBuffer_;  // 読み書きするバッファ配列
	...
	inline void MemoryStream::Write(T* _pArray, const int& _arrayLength)
	{
		...
		std::memcpy(
			...
			reinterpret_cast<mtnet::Byte*>(_pArray),
			size);
	...
	inline T MemoryStream::Read()
	{
		assert((currentIndex + sizeof(T)) < BUFFER_SIZE  // 読み込んでもアンダーランしない
			&& "buffer under run @MemoryStream::Read");

		mtnet::Byte* pickBuffer = new mtnet::Byte[sizeof(T)];  // 取り出し用バッファ用意
		std::memcpy(pickBuffer, &(pBuffer_[currentIndex]), sizeof(T));  // 取り出す
		currentIndex += sizeof(T);  // サイズ分進める
